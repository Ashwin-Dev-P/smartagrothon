...
//get consumers
const getConsumersRepository = async () => {
	const filter = { type: 1 };
	return await UserModel.find(filter)
		.select("username address phone_number email createdAt updatedAt")
		.lean();
};

//get farmer details
const getFarmerDetailsRepository = async (farmer_id) => {
	const select =
		"address phone_number username email createdAt updatedAt location";
	return await UserModel.findById(farmer_id).select(select).lean();
};
